...
// Add Google Fonts and Font Awesome
	function load_fonts() {
		wp_register_style('google-fonts', 'http' . ($_SERVER['SERVER_PORT'] == 443 ? 's' : '') . '://fonts.googleapis.com/css?family=Yanone+Kaffeesatz:300,400,200|Lora:400,400italic|PT+Sans+Narrow:400,700');
...
function one_page_theme_scripts() {
  ...
  wp_enqueue_script(
	...
	wp_enqueue_script( 
		'one-page-theme-navigation', 
		get_template_directory_uri() . '/js/navigation.js', 
		array(), //dependencies
		'20120206', //version number
		true //load in footer
	);

	wp_enqueue_script( 
		'one-page-theme-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', 
		array(), //dependencies
		'20130115', //version number
		true //load in footer
	);

	wp_enqueue_script( 
		'one-page-theme-custom', 
	...
	wp_enqueue_script( 
		'one-page-theme-custom', 
		get_template_directory_uri() . 'hamburgler.js', 
		array('jquery'), //dependencies
		null, //version number
		true //load in footer
	);
